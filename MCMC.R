...
## 예제 10.2
M=3000; m=500
mu0=10; sigsq0=25; a=0.5; b=1 # mu~N(0,5^2), sigma~IG(0.5,1)
x=c(10,13,15,11,9,18,20,17,23,21)
n=length(x) 
xbar=mean(x) #데이터의 평균
var.x=var(x) #데이터의 분산
THETA = matrix(nrow=M, ncol=2) # 시뮬레이션을 통해 TEHTA채울 것

#initial value of sigsq
sigsq=var.x # 데이터의 분산을 초기값으로 넣는다

#####simulation#####
for(nsim in 1:M){
  
  #generate mu
  condpost.mu=(sigsq/n*mu0 + sigsq0*xbar)/(sigsq/n+sigsq0) # 사후분포
  condpost.var=1/(1/sigsq0 + n/sigsq) # 사후분포
  mu=rnorm(1,condpost.mu, sqrt(condpost.var)) # 몬테카를로방법으로 난수생성
...
par(mfrow=c(1,2))
plot(density(THETA[m:M,1]), xlab=expression(mu), ylab="marginal posterior", main="")
plot(density(THETA[m:M,2]), xlab=expression(sigma^2),ylab="marginal posterior", main="")
...
plot(density(eta.sim), type='l', xlab=expression(eta), ylab="posterior",main="")
